...
- **Pieces:** bold primaries; each piece *type* owns one color so players learn shapes by hue (straight · curve · ramp · funnel · cup); the Drop point uses a dedicated violet accent
...
1. **Zero instructions** — first run opens on a small pre-built starter track; the Drop button is obvious and irresistible
