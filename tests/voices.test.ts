...
import { detuneFrequency, type GoalNote, VOICES } from "../src/audio/voices";
...
describe("voice spec module", () => {
  ...
  it("gives goal a rising pentatonic three-note arpeggio under 400ms", () => {
    ...
    const notes: GoalNote[] = goal.arpeggio.notes;
    ...
    expect(goal.arpeggio.rootFreq).toBeGreaterThanOrEqual(200);
    expect(goal.arpeggio.rootFreq).toBeLessThanOrEqual(1000);
    expect(goal.arpeggio.peak).toBeGreaterThan(0);
    expect(goal.arpeggio.peak).toBeLessThanOrEqual(0.5);
  });
